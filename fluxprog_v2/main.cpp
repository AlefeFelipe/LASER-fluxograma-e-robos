...
int main(int argc, char **argv)
{
	...
	//inic_fluxprog();
	ALLEGRO_COLOR preto, branco, cinza; //objetos contendo cores
    ...
    ALLEGRO_FONT *fonte; // variável que abriga a fonte usada
    int modo; // se estiver em -1, o programa está sendo construído. Se em 0, está pausado, e se em 1, rodando
    bool blocos_inicio; //monitora o número de blocos de início: se não houver ao menos 1, não é possível começar a rodar o programa;
    int dmouse_x, dmouse_y; //variáveis usadas no controle do mouse
    //bloco* bloco_topo; // se um bloco estiver sendo desenhado acima de todos os outros nos quais há colisão com o mouse, esse bloco será indicado aqui
    float escala; //utilizado para mostrar o nível de zoom ~(não implementado)
    bool bloco_enviado; //controla se o bloco ativo atual enquanto o programa roda já enviou comandos para a memória compartilhada
    bool mouse_clicar[3]; //diz se, entre o último ciclo e o presente, o usuário apertou o botão da esquerda, direita ou meio do mouse
    bool mouse_segurar[3]; //diz se, entre o ciclo anterior e o presente, o usuário não parou de segurar algum dos  botões do mouse
    bool mouse_soltar[3]; //diz se, entre o ciclo anterior e o presente, o usuário soltou algum dos  botões do mouse
    int mouse_x, mouse_y; //indicam a posição atual do mouse nos eixos x e y
    int *command_var, *feedback, *bluetooth; //variaveis que mandam comandos
    float posicao_y;
    float monitor_comprimento, monitor_altura;
    char caractere_pressionado[2];  //o primeiro campo guarda qual caractere foi pressionado, o segundo marca se foi uma tecla normal '\0'
	...
	//modo = desenhando;
	escala = 1;
	mouse_clicar[0] = false;
	mouse_clicar[1] = false;
	mouse_clicar[2] = false;
	mouse_segurar[0] = false;
	mouse_segurar[1] = false;
	mouse_segurar[2] = false;
	mouse_soltar[0] = false;
	mouse_soltar[1] = false;
	mouse_soltar[2] = false;
	posicao_y = 0;
